...
def divide_sets(dataset):
    ...
    train_set = bad_classes[:int(len(bad_classes) * 0.7)] + \
        good_classes[:int(len(good_classes) * 0.7)]
    ...
    test_set = bad_classes[int(len(bad_classes) * 0.8):] + \
        good_classes[:int(len(good_classes) * 0.8):]
...
if __name__ == '__main__':
    ...
    for i, c in enumerate([classifier, classifier2, classifier3]):
        ...
        print(f'Klasifikatorot {i} ima tochnost so vakudacisko mnozesto od {val_acc}')
        if val_acc > max_acc:
            max_acc = val_acc
            final_classifier = c
